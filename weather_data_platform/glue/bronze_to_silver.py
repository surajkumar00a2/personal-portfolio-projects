...
df_extracted = df_raw.select(
    F.to_timestamp(
        F.col("ingestion_metadata.ingestion_timestamp"),
...
df_flattened = df_extracted.select(
    ...
    # Timestamps - CRITICAL: Cast to Long first to handle schema inference ambiguity
    F.when(
        F.col("dt") > 10000000000, F.from_unixtime(F.col("dt").cast(LongType()) / 1000)
    )
    .otherwise(F.from_unixtime(F.col("dt").cast(LongType())))
    .cast(TimestampType())
    .alias("observation_timestamp"),
    F.when(
        F.col("sys.sunrise") > 10000000000,
        F.from_unixtime(F.col("sys.sunrise").cast(LongType()) / 1000),
    )
    .otherwise(F.from_unixtime(F.col("sys.sunrise").cast(LongType())))
    .cast(TimestampType())
    .alias("sunrise_time"),
    F.when(
        F.col("sys.sunset") > 10000000000,
        F.from_unixtime(F.col("sys.sunset").cast(LongType()) / 1000),
    )
    .otherwise(F.from_unixtime(F.col("sys.sunset").cast(LongType())))
    .cast(TimestampType())
    .alias("sunset_time"),
    # Other
    F.col("timezone").alias("timezone_offset_seconds"),
    F.col("cod").alias("api_response_code"),
)
...
df_weather = (
    ...
    .withColumn("weather_main", F.col("weather_array").getItem(0).getField("main"))
    .withColumn(
        "weather_description", F.col("weather_array").getItem(0).getField("description")
    )
    .withColumn("weather_icon", F.col("weather_array").getItem(0).getField("icon"))
    .drop("weather_array")
)
...
df_quality = df_quality.withColumn("schema_drift_detected", F.lit(False))

# Flag volume anomalies (placeholder - would need historical comparison)
df_quality = df_quality.withColumn("is_volume_anomaly", F.lit(False))
...
print(
    f"Records with missing fields: {missing_count} ({missing_count/df_quality.count()*100:.1f}%)"
)

# Deduplication
print("Deduplicating Records...")

window_spec = Window.partitionBy("city_id", "observation_timestamp").orderBy(
    F.col("ingestion_timestamp").desc()
)

df_deduped = (
    df_quality.withColumn("row_number", F.row_number().over(window_spec))
...
df_final = (
    df_deduped.withColumn("year", F.year(F.col("observation_timestamp")))
...
print(
    f" Temperature range: {temp_stats['min_temp']:.1f} - {temp_stats['max_temp']:.1f} Celsius"
)
...
df_final.write.mode("append").partitionBy("year", "month", "day").parquet(
    SILVER_PATH, compression="snappy"
)
